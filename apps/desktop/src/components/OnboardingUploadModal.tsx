...
export interface OnboardingUploadModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  inspectionData: DatasetInspection | null;
    ...
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <Dialog.Portal>
        ...
        >
              ...
              <div class="space-y-4 flex-1 flex flex-col">
                <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
                  <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                      {t().onboardingModalSubtitle}
                    </p>
                  </div>
                  <Dialog.Close class="text-slate-400 hover:text-slate-200 p-1 rounded-lg hover:bg-slate-800 transition-colors cursor-pointer">
                    <X size={16} />
                  </Dialog.Close>
                </div>
                ...
                >
                  ...
                  <div class="w-16 h-16 rounded-2xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mx-auto shadow-xs">
                    <Show when={props.uploading} fallback={<CloudUpload size={32} />}>
                      <CircularProgress progress={0.65} size={36} strokeWidth={4} color="#10b981" />
                    </Show>
                  </div>
                  <div class="space-y-1.5 max-w-md mx-auto">
                    <p class="text-sm md:text-base font-bold text-slate-900 dark:text-slate-100">
                      {props.uploading
                        ? t().onboardingDropzoneInspecting
                        : props.isDragging
                          ? t().onboardingDropzoneDrop
                          : t().onboardingDropzonePrompt}
                    </p>
                    <p class="text-xs text-slate-500 dark:text-slate-400">
                      {t().onboardingDropzoneFormats}
                    </p>
                    <p class="text-[11px] text-emerald-600 dark:text-emerald-400 font-medium pt-1">
                      {t().onboardingDropzoneMapperHint}
                    </p>
                  </div>
                ...
                <Show when={props.uploadError}>
                  <div class="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-600 dark:text-rose-300 text-xs flex items-center justify-between font-mono">
                    <span>{props.uploadError}</span>
                    <button
                      type="button"
                      onClick={props.onClearUploadError}
                      class="font-bold text-rose-500 hover:text-rose-700 cursor-pointer"
                    >
                      ×
                    </button>
                  </div>
          ...
          >
            <ColumnMapperCanvas
              inspection={props.inspectionData!}
              rawBytes={props.uploadedBytes ?? undefined}
              filePath={props.uploadedFilePath ?? undefined}
