...
export interface RunDeleteDialogProps {
  runId: string | null;
  deleting: boolean;
  onClose: () => void;
  onConfirm: () => void;
...
export const RunDeleteDialog: Component<RunDeleteDialogProps> = (props) => {
  const t = () => catalogs[view.lang];

  return (
    <Dialog
      open={props.runId !== null}
      onOpenChange={(open) => {
        ...
        <Dialog.Content class="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md w-[92vw] p-6 shadow-2xl space-y-4 focus:outline-none text-slate-900 dark:text-slate-100">
          <div class="flex items-center space-x-3 text-rose-600 dark:text-rose-400">
            ...
            <div>
              <Dialog.Label class="text-base font-bold text-slate-900 dark:text-slate-100">
                {t().runsDeleteDialogTitle}
              </Dialog.Label>
              <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                {t().runsDeleteDialogWarning}
              </p>
            </div>
          </div>

          <p class="text-xs text-slate-800 dark:text-slate-300 font-mono bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-800 break-all">
            run_id: {props.runId}
          </p>

          <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            {t().runsDeleteDialogDesc}
          </p>
          ...
          <div class="flex items-center justify-end space-x-2 pt-2">
            <Dialog.Close
              ...
              class="px-3.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer disabled:opacity-50"
            >
              {t().runsDeleteCancel}
            </Dialog.Close>
            <button
            ...
            >
              {props.deleting ? t().runsDeleting : t().runsDeletePermanent}
            </button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
