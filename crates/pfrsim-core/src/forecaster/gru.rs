...
use burn::nn::{Gru, GruConfig, Linear, LinearConfig};
...
#[derive(Module, Debug)]
struct GruNet<B: burn::tensor::backend::Backend> {
    gru: Gru<B>,
    linear: Linear<B>,
    hidden_dim: usize,
}

impl<B: burn::tensor::backend::Backend> GruNet<B> {
    fn new(input_dim: usize, hidden_dim: usize, device: &B::Device) -> Self {
        let gru = GruConfig::new(input_dim, hidden_dim, true).init::<B>(device);
    ...
    fn forward(&self, x: Tensor<B, 3>) -> Tensor<B, 2> {
        ...
        let out = self.gru.forward(x.clone(), None);
        let last = out
...
macro_rules! define_fit_and_forecast_gru {
    ($fn_name:ident, $backend:ty, $device_type:ty) => {
        fn $fn_name(
            ...
            let mut model = GruNet::<$backend>::new(1, hidden_dim, device);
            let mut optim = AdamConfig::new().init::<$backend, GruNet<$backend>>();
...
define_fit_and_forecast_gru!(
    fit_and_forecast_gru_cpu,
    Autodiff<NdArray<f32>>,
    NdArrayDevice
);
define_fit_and_forecast_gru!(fit_and_forecast_gru_gpu, Autodiff<Wgpu>, WgpuDevice);
...
impl Forecaster for GruForecaster {
    ...
    fn forecast(
        ...
        let look_back = config.gru.as_ref().map(|c| c.look_back).unwrap_or(12);
        let epochs = config.gru.as_ref().map(|c| c.epochs).unwrap_or(100);
        let hidden_dim = config
            .gru
            .as_ref()
            .and_then(|c| c.layer_units.first().copied())
            .unwrap_or(16);
        let batch_size = config.gru.as_ref().map(|c| c.batch_size).unwrap_or(32);
        let learning_rate = config.gru.as_ref().map(|c| c.learning_rate).unwrap_or(0.02);
        ...
        let fit_var = |series: &[f64], name: &str| {
            if let Some(dev) = &gpu_device {
                match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    ...
                    Ok(res) => res,
                    Err(_) => fit_and_forecast_gru_cpu(
                        series,
                        h,
                        look_back,
                        epochs,
...
mod tests {
    ...
    fn test_gru_forecast() {
        ...
        let (fc, info, _) =
            fit_and_forecast_gru_cpu(&series, 4, 3, 15, 16, 32, 0.02, "WT", &cpu_dev, None);
        for &v in &fc {
            assert!(!v.is_nan());
        }
    ...
    #[test]
    fn test_gru_full_batch_forecast() {
        let series = vec![10.0, 11.0, 12.0, 13.0, 14.0, 15.0, 16.0, 17.0];
        let cpu_dev = Default::default();
        // batch_size_param = 0 (Full batch)
        let (fc, info, _) =
            fit_and_forecast_gru_cpu(&series, 4, 3, 10, 16, 0, 0.02, "WT", &cpu_dev, None);
        assert_eq!(fc.len(), 4);
        for &v in &fc {
            assert!(!v.is_nan());
        }
